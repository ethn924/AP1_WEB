...
// Vérifier que l'utilisateur a accès au CR
...
if (isset($_GET['restore']) && !empty($_GET['restore'])) {
    $version_id = intval($_GET['restore']);
    
    if (restaurerVersionCR($version_id, $cr_id, $_SESSION['Sid'])) {
        $message = "La version a été restaurée avec succès.";
        // Recharger les données du CR
        $result_cr = mysqli_query($bdd, $query_cr);
        $cr = mysqli_fetch_assoc($result_cr);
    } else {
                ...
                <?php foreach ($versions as $version): ?>
                    <div class="version-item <?php echo $version['numero_version'] == $cr['num_version'] ? 'current' : ''; ?>">
                        ...
                        <div class="version-meta">
                            ...
                            <strong>Description:</strong> <?php echo htmlspecialchars(substr($version['description'], 0, 100)); ?>...
                        </div>
