...
if ($groupe_id) { $groupe_condition = " AND c.groupe_id = $groupe_id"; } else { $groupe_condition = ""; }
$query_stats = "SELECT
s.statut,
COUNT(*) as nombre
FROM cr c
...
$total_crs = 0;
while ($row = mysqli_fetch_assoc($result_stats)) { if ($row['statut']) { $stats_by_status[$row['statut']] = $row['nombre']; }
...
$monthly_stats = array();
while ($row = mysqli_fetch_assoc($result_monthly)) { $monthly_stats[] = $row; }
$rappels_query = "";
if ($groupe_id) { $rappels_query = "SELECT * FROM rappels_soumission WHERE groupe_id = $groupe_id AND actif = 1 ORDER BY date_limite ASC";
    $result_rappels = mysqli_query($bdd, $rappels_query);
    $rappels = array();
while ($row = mysqli_fetch_assoc($result_rappels)) { $rappels[] = $row; } } else { $rappels = array(); }
?>
<!DOCTYPE html>
<html lang="fr">
    <head>
    ...
    .stat-box { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
        ...
        border-radius: 8px;
        text-align: center; }
    .stat-box.soumis { background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); }
    .stat-box.evalues { background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%); }
    .stat-box.approuves { background: linear-gradient(135deg, #43e97b 0%, #38f9d7 100%); }
    ...
    .table-stats th { background-color: #f8f9fa;
        ...
        font-weight: bold; }
    .table-stats td { padding: 12px;
        border-bottom: 1px solid #ddd; }
    .table-stats tr:hover { background-color: #f8f9fa; }
    .percentage-bar { background-color: #e9ecef;
        height: 20px;
        border-radius: 3px;
        overflow: hidden;
        margin: 5px 0; }
    .percentage-fill { background-color: #007bff;
        height: 100%;
        display: flex;
        align-items: center;
    ...
    h2 { border-bottom: 2px solid #eee;
        padding-bottom: 10px; }
    .rappels-list { background: #fff3cd;
        padding: 15px;
        border-radius: 4px;
        border-left: 4px solid #ffc107; }
    .rappel-item { padding: 8px 0;
...
foreach ($stats_by_status as $status => $count) { $labels[] = ucfirst($status);
    $data[] = $count;
$colors[] = $statusColors[$status] ?? '#007bff'; }
    ?>
    const statusCtx = document.getElementById('statusChart').getContext('2d');
    new Chart(statusCtx, { type: 'doughnut',
 data: { labels: <?php echo json_encode($labels); ?>,
 datasets: [ { data: <?php echo json_encode($data); ?>,
