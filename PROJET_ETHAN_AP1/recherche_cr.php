...
$professeurs = array();
while ($row = mysqli_fetch_assoc($result_profs)) { $professeurs[] = $row; }
if ($_SERVER['REQUEST_METHOD'] === 'POST' || isset($_GET['recherche'])) { $filtres = array();
    if (!empty($_POST['titre'] ?? $_GET['titre'] ?? '')) { $filtres['titre'] = $_POST['titre'] ?? $_GET['titre']; }
    if (!empty($_POST['statut'] ?? $_GET['statut'] ?? '')) { $filtres['statut'] = $_POST['statut'] ?? $_GET['statut']; }
    if (!empty($_POST['date_debut'] ?? $_GET['date_debut'] ?? '')) { $filtres['date_debut'] = $_POST['date_debut'] ?? $_GET['date_debut']; }
    if (!empty($_POST['date_fin'] ?? $_GET['date_fin'] ?? '')) { $filtres['date_fin'] = $_POST['date_fin'] ?? $_GET['date_fin']; }
    if (!empty($_POST['groupe_id'] ?? $_GET['groupe_id'] ?? '')) { $filtres['groupe_id'] = $_POST['groupe_id'] ?? $_GET['groupe_id']; }
    if (!empty($_POST['professeur_id'] ?? $_GET['professeur_id'] ?? '')) { $filtres['professeur_id'] = $_POST['professeur_id'] ?? $_GET['professeur_id']; }
    if ($_SESSION['Stype'] == 0) { $filtres['utilisateur_id'] = $_SESSION['Sid']; }
    if (!empty($filtres)) { $resultats = rechercherCR($filtres);
        $filtres_appliques = true;
if (count($resultats) > 0) { $message_recherche = count($resultats) . ' résultat(s) trouvé(s)'; } else { $message_recherche = 'Aucun résultat ne correspond à votre recherche.'; } } }
?>
<!DOCTYPE html>
<html lang="fr">
    <head>
    ...
    .btn-search-group { display: flex;
        gap: 10px;
        align-items: flex-end; }
    .btn-search-group button { flex: 1; }
    .alert { padding: 15px;
        border-radius: 4px;
        margin-bottom: 20px;
    ...
    .result-title { font-weight: bold;
        font-size: 16px;
        color: #333; }
    .result-meta { color: #666;
        font-size: 12px;
        margin-bottom: 8px; }
    .result-description { color: #444;
        margin-bottom: 10px; }
    ...
    .status-badge { display: inline-block;
        ...
        font-size: 11px;
        font-weight: bold; }
    .status-brouillon { background-color: #e2e3e5;
        color: #383d41; }
    .status-soumis { background-color: #fff3cd;
        color: #856404; }
    .status-evalue { background-color: #cce5ff;
        color: #004085; }
    .status-approuve { background-color: #d4edda;
        color: #155724; }
    h1 { color: #333; }
    h2 { color: #333;
