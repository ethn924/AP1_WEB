...
$error = '';
$modele = null;
if (isset($_GET['delete']) && !empty($_GET['delete'])) { $id = intval($_GET['delete']);
    $delete_query = "DELETE FROM modeles_cr WHERE id = $id AND professeur_id = {$_SESSION['Sid']}";
if (mysqli_query($bdd, $delete_query)) { $message = "Le modèle a été supprimé avec succès."; } else { $error = "Erreur lors de la suppression du modèle."; } }
if (isset($_GET['edit']) && !empty($_GET['edit'])) { $id = intval($_GET['edit']);
    $edit_query = "SELECT * FROM modeles_cr WHERE id = $id AND professeur_id = {$_SESSION['Sid']}";
    $edit_result = mysqli_query($bdd, $edit_query);
if (mysqli_num_rows($edit_result) > 0) { $modele = mysqli_fetch_assoc($edit_result); } }
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_modele'])) { $titre = mysqli_real_escape_string($bdd, $_POST['titre']);
    ...
    if (empty($titre)) { $error = "Le titre du modèle est obligatoire."; } else { if (isset($_POST['modele_id']) && !empty($_POST['modele_id'])) { $modele_id = intval($_POST['modele_id']);
            $update_query = "UPDATE modeles_cr
            SET titre = '$titre',
            description = '$description',
            contenu_html = '$contenu_html'
            WHERE id = $modele_id AND professeur_id = $professeur_id";
            if (mysqli_query($bdd, $update_query)) { $message = "Le modèle a été mis à jour avec succès.";
                header("Location: gestion_modeles.php?edit=$modele_id&success=1");
            exit(); } else { $error = "Erreur lors de la mise à jour du modèle: " . mysqli_error($bdd); } } else { $insert_query = "INSERT INTO modeles_cr (titre, description, contenu_html, professeur_id)
            VALUES ('$titre', '$description', '$contenu_html', $professeur_id)";
            if (mysqli_query($bdd, $insert_query)) { $message = "Le modèle a été créé avec succès.";
                header("Location: gestion_modeles.php?success=1");
exit(); } else { $error = "Erreur lors de la création du modèle: " . mysqli_error($bdd); } } } }
...
if (isset($_GET['success']) && $_GET['success'] == 1) { $message = isset($_GET['edit']) ? "Le modèle a été mis à jour avec succès." : "Le modèle a été créé avec succès."; }
?>
<!DOCTYPE html>
<html lang="fr">
    <head>
