...
if (!$modele_id) { header("Location: gestion_modeles.php");
exit(); }
...
if (!$modele) { header("Location: gestion_modeles.php");
exit(); }
$message = '';
$error = '';
if (isset($_GET['delete']) && !empty($_GET['delete'])) { $item_id = intval($_GET['delete']);
    $query = "DELETE FROM checklists_modeles WHERE id = $item_id AND modele_id = $modele_id";
if (mysqli_query($bdd, $query)) { $message = "L'élément a été supprimé avec succès."; } else { $error = "Erreur lors de la suppression."; } }
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_item'])) { $item_texte = mysqli_real_escape_string($bdd, $_POST['item_texte']);
...
if (empty($item_texte)) { $error = "Le texte de l'élément est obligatoire."; } else { if (ajouterChecklistModele($modele_id, $item_texte, $ordre, $obligatoire, $description_aide)) { $message = "L'élément a été ajouté avec succès."; } else { $error = "Erreur lors de l'ajout de l'élément."; } } }
...
<html lang="fr">
    ...
    .item-title { font-weight: bold;
        flex: 1; }
    .item-ordre { background: #007bff;
        color: white;
        padding: 3px 8px;
        border-radius: 3px;
        font-size: 12px;
        margin-right: 10px; }
    .item-badge { display: inline-block;
        background: #dc3545;
        color: white;
        padding: 3px 8px;
        border-radius: 3px;
 ...
 <input type="number" id="ordre" name="ordre" class="form-control" value="<?php echo count($checklist) + 1; ?>" min="1">
                    </div>
                    <div class="form-group">
                        <label>
