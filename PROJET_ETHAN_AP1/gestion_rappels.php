...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_rappel'])) { $groupe_id = intval($_POST['groupe_id']);
...
if (empty($titre) || empty($date_limite)) { $error = "Le titre et la date limite sont obligatoires."; } else { if (creerRappelSoumission($groupe_id, $date_limite, $titre, $description, $_SESSION['Sid'])) { $message = "Le rappel a été créé avec succès."; } else { $error = "Erreur lors de la création du rappel."; } } }
if (isset($_GET['delete']) && !empty($_GET['delete'])) { $rappel_id = intval($_GET['delete']);
    $query = "DELETE FROM rappels_soumission WHERE id = $rappel_id AND professeur_id = {$_SESSION['Sid']}";
if (mysqli_query($bdd, $query)) { $message = "Le rappel a été supprimé avec succès."; } else { $error = "Erreur lors de la suppression du rappel."; } }
if (isset($_GET['deactivate']) && !empty($_GET['deactivate'])) { $rappel_id = intval($_GET['deactivate']);
    $query = "UPDATE rappels_soumission SET actif = 0 WHERE id = $rappel_id AND professeur_id = {$_SESSION['Sid']}";
if (mysqli_query($bdd, $query)) { $message = "Le rappel a été désactivé."; } else { $error = "Erreur lors de la désactivation du rappel."; } }
...
$rappels = array();
while ($row = mysqli_fetch_assoc($result_rappels)) { $rappels[] = $row; }
?>
<!DOCTYPE html>
<html lang="fr">
    <head>
    ...
    .rappel-item { background: #f8f9fa;
        border-left: 4px solid #ffc107;
        padding: 15px;
        margin-bottom: 15px;
        border-radius: 4px; }
    .rappel-item.expired { border-left-color: #dc3545;
        background: #ffe6e6; }
    .rappel-item.inactive { opacity: 0.5; }
    .rappel-header { display: flex;
        justify-content: space-between;
        align-items: center;
        margin-bottom: 10px; }
    .rappel-title { font-weight: bold;
        font-size: 16px; }
    .rappel-status { display: inline-block;
        padding: 4px 8px;
        border-radius: 3px;
        font-size: 11px;
        font-weight: bold; }
    .rappel-status.active { background-color: #d4edda;
        color: #155724; }
    .rappel-status.inactive { background-color: #e2e3e5;
        color: #383d41; }
    .rappel-status.expired { background-color: #f8d7da;
        color: #721c24; }
    .rappel-meta { color: #666;
        font-size: 12px;
        margin-bottom: 8px; }
    .rappel-description { color: #444;
        margin-bottom: 10px; }
    .rappel-actions { display: flex;
        gap: 10px; }
    .rappel-actions a { font-size: 12px;
        padding: 5px 10px; }
    h1, h2 { color: #333; }
...
$classes = 'rappel-item';
if ($est_expiry) $classes .= ' expired';
if ($est_inactif) $classes .= ' inactive';
                ?>
 ...
 <span class="rappel-status <?php echo $est_inactif ? 'inactive' : ($est_expiry ? 'expired' : 'active'); ?>">
 ...
 <?php endif; ?>
 <a href="gestion_rappels.php?delete=<?php echo $rappel['id']; ?>" 
